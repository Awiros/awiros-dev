...
public:
    ...
    void init(int t_port, int t_quality) {
      sock = INVALID_SOCKET;
      timeout = TIMEOUT_M;
      quality = t_quality;
      port = t_port;
    ...
    bool open()
    {
        ...
        int enable = 1;
        if (setsockopt(sock, SOCK_STREAM, SO_REUSEPORT, &enable, sizeof(int)) < 0)
            cerr << "setsockopt(SO_REUSEADDR) failed" <<std::endl;
    ...
    void stop(){
    	bool out = this->release();
        //std::cout<<"[Socket Status(stop)] :"<<out<<std::endl;
